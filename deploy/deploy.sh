...
#   PI_HOST=192.168.0.51 ./deploy/deploy.sh
#
# Requires: rsync, ssh
# ─────────────────────────────────────────────────────────────────────────────
set -euo pipefail
...
if [ -f "$CF_TOKEN_FILE" ]; then
    ...
    PURGE=$(curl -s -X POST "https://api.cloudflare.com/client/v4/zones/8a326892e9e217e292faddd69988b7a0/purge_cache" \
