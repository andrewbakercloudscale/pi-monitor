...
## Infrastructure
- **Pi IP**: `192.168.0.51`
- **SSH**: `ssh -i ~/.ssh/pi_key pi@192.168.0.51`
...
rsync -az --delete -e "ssh -i ~/.ssh/pi_key" out/ pi@192.168.0.51:/opt/pi-monitor/frontend/
```

### Backend (single file)
```bash
scp -i ~/.ssh/pi_key api/<file>.py pi@192.168.0.51:/opt/pi-monitor/api/<file>.py
ssh -i ~/.ssh/pi_key pi@192.168.0.51 "sudo systemctl restart pi-monitor-api"
```
